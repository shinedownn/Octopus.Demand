    ...
    public class ElasticSearchManager : IElasticSearch
    {
            ...
            where T : class
        {
            ...
            var searchResponse = await elasticClient.SearchAsync<T>(s => s
                ...
                .Query(q => q.Match(m=>m.Field(fieldParameters.FieldName)) && q.QueryString(x=>x.Query("*"+fieldParameters.Value+"*"))));
                //.Query(q => q.Match(
            ...
            where T : class
        {
            ...
            var searchResponse = await elasticClient.SearchAsync<T>(s => s
                ...
                //.MatchAll(m => m.Field(o => o.Type).Query("Hotel"))
                .Query(a => a.QueryString(c => c
